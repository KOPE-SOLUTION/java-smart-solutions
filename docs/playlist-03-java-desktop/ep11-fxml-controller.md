...
การ์ด `Sensor ผิดปกติ` นับเฉพาะสถานะ `WARNING` จึงแสดง `1` จาก `M-002` ส่วนการ์ด `ต้องบำรุงทั้งหมด` นับทั้ง Sensor ผิดปกติและชั่วโมงทำงาน จึงแสดง `2` จาก `M-002` และ `M-003`
